...
```{r echo = FALSE}
ggplot() + 
  geom_line(aes(x = spain_data$days_since_first_death, y = confidence_spain[1,6]*exp(confidence_spain[2,2]*spain_data$days_since_first_death), color = "Spain")) + 
  geom_line(aes(x = italy_data$days_since_first_death, y = confidence_italy[1,6]*exp(confidence_italy[2,2]*italy_data$days_since_first_death), color = "Italy")) +
  labs(
    x = "Days Since First Death",
    y = "Predicted Number of Deaths",
    title = "Comparison of Predicted Deaths in Spain and Italy during Covid",
    color = "Country"
  )
```  
...
set.seed(42)
boot_milk = do(1000)*lm(log(price) ~ log(sales), data = resample(milk))
...
The estimated value, using power law, for the Price Elasticity of Demand (PED) for Milk is -0.48, and the 95% confidence interval for the PED for Milk is -0.523 to -0.44. Since PED is a ratio of percentage change in quantity and percentage change in price, and the absolute values of the PED values are all less than 1, this means that the product, which in this case is milk, is *inelastic*. This means a 1% increase in price leads to less than a 1% decrease in sales, which can be observed from the -0.523% and the -0.44% being both less than -1%. I estimated the elasticity by using a power law model (linear regression with both axes on log scale), setting the x to be log(price) and the y to be log(sales). Getting the correlation coefficient (as similarly done in the previous problems) gave the PED, which was -0.48. 